...
- Explain about the SOLID rules plus a number of the additional rules given in Effective Unit Testing focusing on WHY following these rules makes code more testable
  - Solid
    ...
    - I, interface segration principle: det er bedre at have mange små interfaces istedet for et stort. det gør kode nemmere at teste fordi det er nemmere at lave en mock udfra et lille interface
...
- Why would you consider alternative matchers like for example Hamcrest matchers in a JUnit project? Provide examples to demonstrate, how to set up a project to use Hamcrest, - the effect of using Hamcrest matchers compared to JUnit's basic Asserts.
  - Brugen af Hamcrest giver mulighed for at lave customized assertions, det ville altså sige at vi selv kan bestemmer hvordan assertion ser ud. Hamcrest er declarative (forklar hvad vi ville have) frem for imperative (forklare hvad den skal gøre for at få det vi ville have). 
  - Hamcrest giver en højere readability (Hamcrest) AssertThat(aString.length,IsEven()); (Normal JUnit) AssertTrue(a.String.length % 2 == 0);
  - Hamcrest error messages er også mere specifikke fordi vi netop fortæller den hvad vi ville have, og den kan så reportere tilbage om vi fik det vi ville have. normal JUnit derimod ved ikke hvor fejlen nødvendigvis er, og reportere så bare at assertionen laver en fejl.


- Explain why testers just love the Dependency Injection Pattern
  - Dependency injection giver en meget større kontrol over tingene. for hvis dependencien bare ligger i objekt hvor man ikke kan teste det. kan det være svært at mocke væk. hvorimod hvis det kommer med som en parameter, kan vi bare lave en mock (som vi har control over) over giver den med.
